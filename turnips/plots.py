...
from turnips import MetaModel, TimePeriod, Markov

def plot_models_range(name, models, previous, add_points=False):
    '''
    ...
    for day in range(2, 14):
        if a_model.timeline[TimePeriod(day)].price.is_atomic:
            ...
            if day == 2:
                ...
                # plot known data
                vals = [a_model.timeline[TimePeriod(day)].price.value for day in range(2, final_precise + 1)]
                days = range(2, final_precise + 1)
                plt.plot(days, vals, c='black')
            break

    model_counts = Counter(x.shortname for x in models)
    remaining_model_types = model_counts.keys()
    remaining_probability = sum(Markov[previous][rem_mod] for rem_mod in remaining_model_types)
    adjusted_priors = {model: Markov[previous][model] / remaining_probability for model in model_counts.keys()}

    days = range(final_precise, 14)
    for model in models:
        low_vals = [model.timeline[TimePeriod(day)].price.lower for day in range(final_precise, 14)]
        high_vals = [model.timeline[TimePeriod(day)].price.upper for day in range(final_precise, 14)]

        if previous != 'Unknown':
            alpha = adjusted_priors[model.shortname] / model_counts[model.shortname]
        else:
            alpha = 1 / len(models)

        plt.fill_between(days, low_vals, high_vals, alpha=alpha, color=colors[model.shortname])
    ...
    # cosmetics
    msummary = '+'.join(['{}_{{{}}}^{{{:.2f}}}'.format(t, l, adjusted_priors[l]) for l, t in model_counts.items()])
    ax.set_title(f'Island {name}: ${len(models)}_{{total}}={msummary}$, Last: {previous}')
    ax.set_ylabel('Turnip Price')
    ax.set_xticklabels(['Mon AM', 'Mon PM', 'Tue AM', 'Tue PM', 'Wed AM', 'Wed PM', 'Thu AM',
...
def global_plot(island_models):
    ...
    fig, ax = plt.subplots(num_ticks, sharex=True)

    for i, (time, pricecounts) in enumerate(hist.items()):
        # Determine likeliest price(s)
        #n_possibilities = sum(pricecounts.values())
        #likelihoods = ({k: v/n_possibilities for k, v in sorted(pricecounts.items(), key=lambda item: 1/item[0])})
